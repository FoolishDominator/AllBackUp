    ...
    switch (gettype(fileheader)) {
      ...
      case FILETYPE_HARDLNK:
        ...
        real_linkpath.assign(target_path.substr(0, pos + 1) +
                             fileheader.linkpath);
        if (link(real_linkpath.c_str(), target_path.c_str()) == 0) wds = true;
        BLUE_WITHOUTENTER("HARDLINK|");
        std::cout << target_path;
        if (wds)
          GREEN("|TRUE");
      ...
      case FILETYPE_SYMLNK:
        if (symlink(fileheader.linkpath, target_path.c_str()) == 0) wds = true;
        BLUE_WITHOUTENTER("SYMLINK|");
        std::cout << target_path;
        if (wds)
          GREEN("|TRUE");
      ...
      case FILETYPE_FIFO:
        mode = 0666;
        if (mkfifo(target_path.c_str(), mode) == 0) wds = true;
        BLUE_WITHOUTENTER("FIFO|");
        std::cout << target_path;
        if (wds)
          GREEN("|TRUE");
