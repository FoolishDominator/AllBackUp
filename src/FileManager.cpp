      ...
      switch (file->get_filetype()) {
        case FILETYPE_HARDLNK:
          if (this->inode_table.count(file->get_stat().st_ino)) {
            ...
            std::filesystem::path to(this->inode_table[file->get_stat().st_ino]
                                         .c_str());  // 目标指向绝对路径
            file->set_linkpath(std::filesystem::relative(
                to, std::filesystem::absolute(from.parent_path())));
          } else {
        ...
        case FILETYPE_SYMLNK:
          ...
          if (len != -1) {
            ...
            std::cout << "readlink error" << file->get_path().c_str()
                      << std::endl;
          }
          break;
